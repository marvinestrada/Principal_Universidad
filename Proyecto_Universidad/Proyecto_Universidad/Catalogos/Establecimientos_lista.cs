...
{
    public partial class Establecimientos_lista : Form
    {
        public Establecimientos_lista()
        {
            InitializeComponent();
        }
        ...
        {
            Establecimientos_form ventana = new Establecimientos_form();
            ventana.ShowDialog();
            int cod = ventana.id;
            ventana.Dispose();
            if (cod != 0)
        ...
        private void bot_actualizar_Click(object sender, EventArgs e)
        {
           // MessageBox.Show(grid_datos.CurrentRow.Cells[1].Value.ToString());
            Establecimientos_form ventana = new Establecimientos_form(Convert.ToInt32(grid_datos.CurrentRow.Cells[0].Value), grid_datos.CurrentRow.Cells[1].Value.ToString(), grid_datos.CurrentRow.Cells[2].Value.ToString());
            ventana.ShowDialog();
            ventana.Dispose();
